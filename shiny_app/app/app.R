...
"\n\n",
    ...
    list(
      ...
      # Specify which columns to apply polynomial transformation to
      poly_vars <- c("age","Kilometres_num","Cylinders")
      
      # Create squared terms only for selected variables
      for (var in poly_vars) {
        train_data[[var]] <- train_data[[var]]^2
        test_data[[var]] <- test_data[[var]]^2
      }
      ...
      # Train the polynomial model using cross-validation
      poly_model <- train(
        model_formula,
        data = train_data,
        method = "lm",
      ...
      train_data$log_Price_pred <- predict(poly_model, newdata = train_data)
      ...
      # Predict on the test data
      test_data$log_Price_pred <- predict(poly_model, newdata = test_data)
      ...
      # --- 3. Plot: Residuals Distribution ---
      model_data <- data.frame(
        Fitted = fitted(poly_model$finalModel),
        Residuals = residuals(poly_model$finalModel)
      )
      ...
      # TRAINING performance metrics
      train_predictions <- predict(poly_model$finalModel, newdata = train_data)
      residuals_train <- train_data$Price_log - train_predictions
      rss_train <- sum(residuals_train^2)
      n_train <- nrow(train_data)
      p <- length(poly_model$finalModel$coefficients)  # Number of predictors incl. intercept
      rse_train <- sqrt(rss_train / (n_train - p))
      log_Price_train <- log(train_data$Price)
      rmse_train <- sqrt(mean((train_data$log_Price_pred - log_Price_train)^2))
