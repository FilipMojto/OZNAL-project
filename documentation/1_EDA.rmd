...
show.boxplot(data = selected_data, cols = PREDICTOR_NAMES_SHORT, target = TARGET)
...
Now we can use scatterplot and try to apply the **smoothing line of Logistic Regression** and attempt to **catch the linearity between target and predictors**.
...
We can see from the plots, that for each predictor the algorithm **fails to converge**. This stems from the already mentioned imbalance between the target classes.
