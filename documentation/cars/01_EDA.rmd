...
*Car prices are heavily influenced by attributes like Brand, Age, Model, etc.*
...
Will need to be converted into numbers using some kind of encoding as we think they influence the price.
...
We already identified some anomalies in data that need to be dealt with for the sake of successful EDA. First it is the missing values.
...
From the values we can tell that the portion ranges from 0% to 0.1%. We can declare that this dataset doesn't contain significant amount of missing values. We will recommend appropriate actions later in this document but now, for the purpose of EDA, we simply remove all rows that contain any missing values.
...
### Brand, Model, Car_Suv, UsedOrNew and Transmission
...
We also need to check whether there are not high correlations between individual predictors. This kind of predictors are duplicate and bring no new information to the model function.
...
-   We detected some portion of outliers -\> Log scale, IQR, etc.

-   For this problem we recommend training **Linear Regression**, **Polynomial Regression** or **Random Forest**.
