...
```{r}
# Takes the column-wise means of that logical matrix. In R, TRUE is coerced to 1 and FALSE to 0, so the mean of each column’s TRUE/FALSE values is really the proportion of TRUE values.
...
target.encode <- function(train_df, test_df, target, col) {
  # we convert the string into symbol
  # it is same but is unqouted, can easily be handled by dplyr
  col_sym <- sym(col)
  encoded_col_name <- paste0(col, "_encoded")

  # Step 1: Calculate mean target per group (train set only)
  # the !! (the “bang‐bang” operator) unquotes that symbol so that group_by() sees it
  # exactly as if you’d written: group_by(Brand)
  avg_target <- train_df %>%
    group_by(!!col_sym) %>%
  ...
  # Step 2: Assign ranks
  # we'll do that by converting the mean_target first into factors, unique() keeps the
  # original orders (otherwise it would go e)
  ...
  # Keeps all rows from train_df adds only selected columns
  # if no match is found, NA is inserted instead
   # still safer to use left join rather than inner join since mindlessly dropping data is dangerous
  train_df <- train_df %>%
...
These are columns that need to be encoded. We simply apply the function defined above on each of them. We are applying the target encoding on training and testing set separately to prevent data leakage.
...
# stratified sampling - the target distribution is maintained
# example: c(3, 7, 1, 12, …)
# train_index contains indices of rows that are picked evenly
# df[train_index, ] creates a subset of rows which match the indices
# df[-train_index, ] creates a subset of rows which don't match the indices
...
# works correctly if both datasets have exactly the same columns in the same order
...
# Sys.Date() returns actual date
# format() extracts only years
...
# we extract only the litres, all values are in format: X L / 100 km
df <- df %>%
  mutate(
...
Except for the count of Seats or Doors both of these predictors also contain some text (4 doors, etc.). Thus we only keep the numeric values by executing the following code.
...
In this section we check how well the predictors correlate with the target. We utilize Scatterplots for that purpose. We also try to draw a linear line in the plot to see if there is linear relationship.
...
show.correlation.plot <- function(df, target, col){
  ...
  p <- ggplot(df, aes(x = .data[[col]], y = .data[[target]])) +
    ...
    geom_smooth(method = "lm", color = "red", formula = y ~ x) +  # Linear trend line
...
We also need to visualize the proportion of outliers present in our data. To achieve this in a neat and representative way, we are going to utilize Boxplots. We need to bin the target beforehand as it is continuous.
...
show.boxplot <- function(data, cols, target, bins = 5) {
  # binning the data
  # cut() takes a numeric vector (your continuous target variable) and splits it into intervals or bins.
  # include.lowest = TRUE: Ensures that the lowest value of the variable is included in the first interval.
  data <- data %>%
  ...
  plots <- map(cols, ~ggplot(
    ...
    aes(x = target_binned, y = .data[[.x]])) +
      ...
      theme_minimal() +
      # we wrap long bin labels by adding newline to improve readibility
...
We also need to check the distribution of the target variable to see how balanced the dataset is. We have found out that the prices are highly skewed. By applying the log scale we can deal with these extremes values. Thus, we plot two histograms comparing the original distribution against the log-scaled distribution. For better readability, we are going to utilize scale_x_log10() function which keeps the original price values which are displayed on x-axis but the distribution is now scaled.
...
From the plots above we can tell that scaling helped making the distribution more normal. The x-axis is **labeled in original units** for readability, even though the data is plotted on a log scale.
...
-   We detected some portion of missing values -\> Removing is required:
    ...
    c)  '-' in some of the attributes

    d)  in *Cylinders*, *CylindersinEngine*
...
-   We recommend trying the following parameters:
    ...
    a)  **Brand_Model_encoded** (Merging Brand and Model)
