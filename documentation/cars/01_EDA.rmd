...
# 
# target.encode <- function(train_df, test_df, target, col, k = 5) {
#   col_sym <- sym(col)
#   encoded_col_name <- paste0(col, "_encoded")
# 
#   global_mean <- mean(train_df[[target]], na.rm = TRUE)
# 
#   #computing count and mean per category
#   avg_target <- train_df %>%
#     group_by(!!col_sym) %>%
#     summarise(
#       n = n(),
#       mean_target = mean(.data[[target]], na.rm = TRUE),
#       .groups = "drop"
#     )
# 
#   # applying smoothing regularizaiton
#   avg_target <- avg_target %>%
#     mutate(
#       smoothed_target = (n * mean_target + k * global_mean) / (n + k)
#     )
# 
#   # assigning encoded values based on smoothed means
#   avg_target <- avg_target %>%
#     mutate(!!encoded_col_name := as.numeric(factor(smoothed_target, levels = unique(smoothed_target))) - 1)
# 
#   # merging into train and test sets
#   train_df <- train_df %>%
#     left_join(avg_target %>% select(!!col_sym, !!sym(encoded_col_name)), by = col)
# 
#   test_df <- test_df %>%
...
for (col in COLS_TO_ENCODE) {
  encoded <- target.encode(train_data, test_data, target = "Price", col = col, k = 10)
