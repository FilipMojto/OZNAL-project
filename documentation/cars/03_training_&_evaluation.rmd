...
EDA_selected = c('Brand_encoded'Model_encoded', 'Location_encoded', 'Car_Suv_encoded', 'Kilometres_num', 'Cylinders', 'ColourExtInt_encoded', 'FuelType_encoded',
                 'DriveType_encoded')
...
# Extract final model to use base R plotting
par(mfrow = c(1, 1))
plot(lm_model$finalModel)

```

From the plots we can tell that residuals are scattered quite normally around zero. From the QQPlot we can see that they almost perfectly follow the line, though there are deviated values (tails).
